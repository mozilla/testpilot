...
footerLinkAboutHeader = Sobre
footerLinkAboutUs = Sobre nós
footerLinkSupportHeader = Ajuda
footerLinkDownload = Baixe o Firefox
...
landingLegalNoticeWithLinks = Ao prosseguir, você concorda com os <terms-link>termos de uso</terms-link> e com a <privacy-link>política de privacidade</privacy-link> do Test Pilot.
landingMoreExperimentsButton = Mais experimentos
...
experimentListHeaderWithFeatured = Teste todos os nossos experimentos
...
experimentGradReportButton = Relatório conclusivo
...
experimentGradReportReady = Preparamos um relatório conclusivo completo.
experimentGoToLink = Vá para { $title }
startedDate = Início do experimento em: <b>{ DATETIME($startedDate) }</b>
...
retireMessageUpdate = Como quiser. Isso ira desinstalar o Test Pilot. Você pode desabilitar experimentos individualmente através do gerenciador de extensões do Firefox.
...
eolTitleMessage = { $title } terminará em { DATETIME($completedDate) }
...
completedDate = Término do experimento: <b>{ DATETIME($completedDate) }</b>
...
localeNotTranslatedWarningTitle = Este experimento não foi traduzido para o seu idioma ({ $locale_code }).
...
warningGoToFAQDetail = Algo deu errado com o Test Pilot. <a>Consulte o FAQ</a> para saber mais.
...
jsDisabledWarning = Desculpe, o Test Pilot requer JavaScript.
