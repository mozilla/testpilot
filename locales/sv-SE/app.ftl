...
landingLegalNoticeWithLinks = Genom att fortsätta godkänner du <terms-link>användarvillkoren</terms-link> och <privacy-link>sekretesspolicy</privacy-link> för Test Pilot.
landingMoreExperimentsButton = Fler experiment
...
mobileDialogAndroidButton = Ladda ner Android-appen
mobileDialogIOSButton = Ladda ner iOS-appen
...
experimentManuallyDisabled = { $title } inaktiverad i tilläggshanteraren
...
startedDate = Startdatum för experiment: <b>{ DATETIME($startedDate) }</b>
...
eolTitleMessage = { $title } slutar den { DATETIME($completedDate) }
...
completedDate = Slutdatum för experiment: <b>{ DATETIME($completedDate) }</b>
...
eolTitle = Away we go …
eolMessageOne = Efter tre år och mer än 20 experiment, { -product } stänger vi ner för gått
eolMessageTwo =
    Med ert generösa deltagande och feedback, så har vi format { -brand } med fantastiskta nya funktioner som t.e.x <a data-l10n-name="containersLink" title="{ -exp-containers }">{ -exp-containers }</a><a data-l10n-name="activityStreamLink" title="{ -exp-activity-stream }">{ -exp-activity-stream }</a>, and
