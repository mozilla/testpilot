...
sendMeasurements3 = Send には Mozilla の <a href="https://www.mozilla.org/privacy/websites/" target="_blank">ウェブサイトプライバシー通知</a> も適用されます。Send のウェブサイトを訪れる際、あなたの IP アドレスなどの情報が標準的なサーバーログの一部として一時的に記録されます。
sendMeasurements4 = Send は、パフォーマンスやあなたのサービス利用状況に関する情報も収集します。これには、ファイルのアップロード頻度、期限切れ前に Mozilla のサーバーにファイルが保存された期間、ファイル転送に関する何らかのエラー、お使いのブラウザが対応している暗号化プロトコルが含まれます。
