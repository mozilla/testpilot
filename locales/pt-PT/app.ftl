...
menuFileIssue = Submeter um problema
...
mobileDialogAndroidButton = Obter a aplicação Android
mobileDialogIOSButton = Obter a aplicação iOS
...
experimentListHeaderWithFeatured = Experimente todas as nossas experiências
...
noScriptMessage = O Test Pilot requer JavaScript.<br>Pedimos desculpa.
...
eolMessageThree =
    Também criámos ótimas novas experiências de aplicações, como
    <a data-l10n-name="lockboxLink" title="{ -exp-lockbox }">{ -exp-lockbox }</a> e
    <a data-l10n-name="sendLink" title="{ -exp-send }">{ -exp-send }</a>
    que continuam a empurrar o { -brand } para além do navegador. Espere por novos desenvolvimentos interessantes de ambos os projetos em 2019.
eolMessageFour =
    Outras experiências como
    ...
    <a data-l10n-name="priceWiseLink" title={ -exp-price-wise }>{ -exp-price-wise }</a> e
